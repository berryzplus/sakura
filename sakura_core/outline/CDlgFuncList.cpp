...
//		::ShowWindow( hwndTree, SW_HIDE );
		...
		for (i = 0; i < m_pcFuncInfoArr->GetNum(); ++i) {
			...
			item.mask = LVIF_TEXT;
			if (1 == pcFuncInfo->m_nInfo) { item.pszText = const_cast<TCHAR*>(LS(STR_DLGFNCLST_REMARK01)); }
			else
				if (10 == pcFuncInfo->m_nInfo) { item.pszText = const_cast<TCHAR*>(LS(STR_DLGFNCLST_REMARK02)); }
				else
					if (20 == pcFuncInfo->m_nInfo) { item.pszText = const_cast<TCHAR*>(LS(STR_DLGFNCLST_REMARK03)); }
					else
						if (11 == pcFuncInfo->m_nInfo) { item.pszText = const_cast<TCHAR*>(LS(STR_DLGFNCLST_REMARK04)); }
						else
							if (21 == pcFuncInfo->m_nInfo) { item.pszText = const_cast<TCHAR*>(LS(STR_DLGFNCLST_REMARK05)); }
							else
								if (31 == pcFuncInfo->m_nInfo) { item.pszText = const_cast<TCHAR*>(LS(STR_DLGFNCLST_REMARK06)); }
								else
									if (41 == pcFuncInfo->m_nInfo) { item.pszText = const_cast<TCHAR*>(LS(STR_DLGFNCLST_REMARK07)); }
									else
										if (50 == pcFuncInfo->m_nInfo) { item.pszText = const_cast<TCHAR*>(LS(STR_DLGFNCLST_REMARK08)); }
										else
											if (51 == pcFuncInfo->m_nInfo) { item.pszText = const_cast<TCHAR*>(LS(STR_DLGFNCLST_REMARK09)); }
											else
												if (52 == pcFuncInfo->m_nInfo) { item.pszText = const_cast<TCHAR*>(LS(STR_DLGFNCLST_REMARK10)); }
												else {
