...
template<typename CharType, size_t nBufCount>
inline int auto_vsprintf_s(CharType(&buf)[nBufCount], const CharType* format, va_list& v)
{
...
inline int auto_sprintf_s(WCHAR *buf, size_t nBufCount, const WCHAR* format, ...)
{
	va_list v;
	va_start(v, format);
	int n = auto_vsprintf_s(buf, nBufCount, format, v);
	va_end(v);
	return n;
}

template<size_t nBufCount>
inline int auto_sprintf_s(WCHAR(&buf)[nBufCount], const WCHAR* format, ...)
{
	va_list v;
	va_start(v, format);
	int n = auto_vsprintf_s(buf, nBufCount, format, v);
...
inline int auto_snprintf(WCHAR *buf, size_t nMaxCount, const WCHAR* format, ...)
{
	va_list v;
	va_start(v, format);
	int n = auto_vsnprintf(buf, nMaxCount, format, v);
	va_end(v);
	return n;
}
