...
public:
	...
	const char* GetVersion(){		//!< DLLのバージョン情報を取得。m_szMsgを壊す
		if ( IsAvailable() ){
			::sprintf_s(m_szMsg, "PPA.DLL Version %d.%d", m_fnGetVersion() / 100, m_fnGetVersion() % 100);
			return m_szMsg;
		}
		return "";
	}
	...
	static char* GetDeclarations( const MacroFuncInfo& cMacroFuncInfo, char(&szBuffer)[nBufCount] );
